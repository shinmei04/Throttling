...
// ==========================================
// tc コマンドで帯域を制御する前提の通常遷移計測スクリプト
// - ネットワークエミュレーションは使わず、キャッシュ無効化のみ
// - Home から各ページへクリック遷移を 100 回ずつ実施
// - 指標計算は Premeasure_tc.js と同じ
// ==========================================

const TRIAL_COUNT = 100; // 各ターゲットでの計測回数
const OUTPUT_FILE = 'raw_normal_tc_data.csv'; // 保存ファイル名
...
(async () => {
    ...
    console.log(`=== tc帯域制御 前提の通常遷移計測開始: ${TRIAL_COUNT}回計測 (Wait: ${WAIT_TIME}ms) ===`);
